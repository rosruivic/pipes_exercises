...
int	main(void)
{
	...
	int	fd3;							// para el redireccionamto de salida
	...
	else // padre (hijos heredan fd abiertos, cerrar los no necesarios antes de fork)
	{
		...
		else	// padre (cerrar fds que no sean necesarios antes de hacer fork)
		{
			...
			else if (pid == 0)			// hijo 3 - hereda los fd del padre
			{
				...
				close(fd2[0]);
				fd3 = open(FILE_NAME, O_WRONLY);	// añadida para redirec. salida >
				dup2(fd3, STDOUT_FILENO);			// añadida para redirec. salida >
				close(fd3);							// añadida para redirec. salida >
